...
import watertap.property_models.NaCl_T_dep_prop_pack as props_sw
...
def build():
    ...
    # Properties
    m.fs.properties_feed = props_sw.NaClParameterBlock()
    ...
    # Translator block to convert distillate exiting condenser from water to NaCl prop pack
...
def set_operating_conditions(m):
    ...
    m.fs.costing.eq_evaporator_material_factor = Constraint(
        ...
        ((9-3)/(260-35))*(m.fs.evaporator.properties_brine[0].mass_frac_phase_comp["Liq", "NaCl"].value*1e3 - 35) + 3
    )
...
def initialize_system(m, solver=None):
    ...
    m.fs.evaporator.properties_brine[0].mass_frac_phase_comp["Liq", "NaCl"] = (
        m.fs.feed.properties[0].mass_frac_phase_comp["Liq", "NaCl"]
        / (1 - m.fs.recovery[0])
    )
    m.fs.evaporator.properties_brine[0].mass_frac_phase_comp["Liq", "H2O"] = (
        1 - m.fs.evaporator.properties_brine[0].mass_frac_phase_comp["Liq", "NaCl"].value
    )
    m.fs.evaporator.properties_brine[0].flow_mass_phase_comp["Liq", "NaCl"] = (
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "NaCl"]
    )
    ...
    m.fs.separator_feed.mixed_state[0].mass_frac_phase_comp["Liq", "NaCl"]
    ...
    m.fs.separator_feed.hx_brine_cold_state[0].mass_frac_phase_comp["Liq", "NaCl"]
    m.fs.separator_feed.hx_distillate_cold_state[0].mass_frac_phase_comp["Liq", "NaCl"]
    ...
    m.fs.hx_distillate.hot_inlet.flow_mass_phase_comp[0, "Liq", "NaCl"] = 1e-4
    ...
    m.fs.hx_brine.hot_inlet.flow_mass_phase_comp[0, "Liq", "NaCl"] = (
        m.fs.evaporator.properties_brine[0].flow_mass_phase_comp["Liq", "NaCl"]
    )
...
def display_metrics(m):
    ...
    print(
        ...
        % (m.fs.feed.properties[0].mass_frac_phase_comp["Liq", "NaCl"].value * 1e3)
    )
    print(
        "Brine salinity:                           %.2f g/kg"
        % (
            m.fs.evaporator.properties_brine[0].mass_frac_phase_comp["Liq", "NaCl"].value
            * 1e3
        )
    )
    print(
