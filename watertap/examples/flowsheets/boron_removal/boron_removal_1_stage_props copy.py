...
def build():
    ...
    m.fs.boron_2rej = Var(initialize=0.5, bounds=(0, 1))
    m.fs.boron_limit = Param(initialize= 2.4/1000, mutable = True, doc= "boron limit g/ kg w")

    m.fs.boron_1rej.fix(0.5)
    m.fs.boron_2rej.fix(0.97)
    # constraints
   
    m.fs.eq_boron_RO2_quality = Constraint(
        expr=(
            m.fs.boron_feed * (1 - (m.fs.boron_1rej+m.fs.boron_1rej*m.fs.boron_2rej)) <= m.fs.boron_limit
    )
    )
    ...
    m.fs.s02 = Arc(source=m.fs.P1.outlet, destination=m.fs.RO1a.inlet)
    m.fs.s03 = Arc(source=m.fs.RO1a.retentate, destination=m.fs.RO1b.inlet)
    m.fs.s04 = Arc(source=m.fs.RO1a.permeate, destination=m.fs.M2.RO1a)
    m.fs.s05 = Arc(source=m.fs.RO1b.permeate, destination=m.fs.M2.RO1b)
    m.fs.s06 = Arc(source=m.fs.RO1b.retentate, destination=m.fs.disposal.inlet)
    m.fs.s07 = Arc(source=m.fs.M2.outlet, destination=m.fs.product.inlet)
    ...
    m.fs.RO1a.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.costing)
    # m.fs.RO1b.costing = UnitModelCostingBlock(flowsheet_costing_block=m.fs.costing,ro_type=ROType.standard)
    
    m.fs.RO1b.costing.reverse_osmosis.membrane_cost.fix(30*1.2) # 20% increase in membrane cost for boron removal
    ...
    iscale.set_scaling_factor(m.fs.RO1a.area, 1e1)
    iscale.set_scaling_factor(m.fs.RO1b.area, 1e1)
...
def set_operating_conditions(m, water_recovery=0.5, over_pressure=0.3, solver=None):
    ...
    m.fs.RO1a.A_comp.fix(4.2e-12)  # membrane water permeability coefficient [m/s-Pa]
    m.fs.RO1a.B_comp.fix(3.5e-8)  # membrane salt permeability coefficient [m/s]
    m.fs.RO1a.feed_side.channel_height.fix(1e-3)  # channel height [m]
    m.fs.RO1a.feed_side.spacer_porosity.fix(0.97)  # spacer porosity [-]
    m.fs.RO1a.permeate.pressure[0].fix(101325)  # atmospheric pressure [Pa]
    m.fs.RO1a.width.fix(5)  # stage width [m]

    m.fs.RO1b.A_comp.fix(4.2e-12)  # membrane water permeability coefficient [m/s-Pa]
    m.fs.RO1b.B_comp.fix(3.5e-8)  # membrane salt permeability coefficient [m/s]
    m.fs.RO1b.feed_side.channel_height.fix(1e-3)  # channel height [m]
    m.fs.RO1b.feed_side.spacer_porosity.fix(0.97)  # spacer porosity [-]
    m.fs.RO1b.permeate.pressure[0].fix(101325)  # atmospheric pressure [Pa]
    m.fs.RO1b.width.fix(5)  # stage width [m]
   
    m.fs.RO1a.area.fix(25)  # guess area for RO initialization
    m.fs.RO1b.area.fix(25)  # guess area for RO initialization

    m.fs.RO1b.area.setlb(1)
    m.fs.RO1b.area.setub(150)
    m.fs.RO1a.area.setlb(1)
    m.fs.RO1a.area.setub(150)
...
def initialize_system(m, solver=None):
    ...
    # ---initialize ROs---
    # initialize RO1
    m.fs.RO1a.feed_side.properties[0, 0].flow_mass_phase_comp["Liq", "H2O"] = value(
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "H2O"]
    )
    m.fs.RO1a.feed_side.properties[0, 0].flow_mass_phase_comp["Liq", "NaCl"] = value(
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "NaCl"]
    )
    m.fs.RO1a.feed_side.properties[0, 0].temperature = value(
        m.fs.feed.properties[0].temperature
    )
    m.fs.RO1a.feed_side.properties[0, 0].pressure = value(
        m.fs.P1.control_volume.properties_out[0].pressure
    )
    m.fs.RO1a.initialize(optarg=optarg, solver="ipopt-watertap")

    m.fs.RO1b.feed_side.properties[0, 0].flow_mass_phase_comp["Liq", "H2O"] = value(
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "H2O"]
    )
    m.fs.RO1b.feed_side.properties[0, 0].flow_mass_phase_comp["Liq", "NaCl"] = value(
        m.fs.feed.properties[0].flow_mass_phase_comp["Liq", "NaCl"]
    )
    m.fs.RO1b.feed_side.properties[0, 0].temperature = value(
        m.fs.RO1a.feed_side.properties[0, 0].temperature
    )
    m.fs.RO1b.feed_side.properties[0, 0].pressure = value(
        m.fs.RO1a.feed_side.properties[0, 0].pressure
    )
    m.fs.RO1b.initialize(optarg=optarg, solver="ipopt-watertap")
    # unfix guessed area, and fix water recovery - should water recovery be 0.5 for both stages?
    m.fs.RO1a.area.unfix()
    m.fs.RO1a.recovery_mass_phase_comp[0, "Liq", "H2O"].fix(0.5)
    m.fs.RO1b.area.unfix()
    m.fs.RO1b.recovery_mass_phase_comp[0, "Liq", "H2O"].fix(0.5)
...
def optimize_set_up(m):
    ...
    m.fs.RO1a.recovery_mass_phase_comp[0, "Liq", "H2O"].unfix()
    m.fs.RO1b.recovery_mass_phase_comp[0, "Liq", "H2O"].unfix()
    ...
    m.fs.overall_area = Var(initialize=100, bounds=(1,500), doc="Total membrane area m2")
    m.fs.RO_area_eq = Constraint(
        expr=m.fs.RO1b.area == 0.25*m.fs.overall_area
    ) 
    m.fs.RO_area_eq2 = Constraint(
        expr=m.fs.overall_area == m.fs.RO1a.area + m.fs.RO1b.area
    )  # total membrane area [m2]

    m.fs.RO1a.width.unfix()
    m.fs.RO1a.width.setlb(1)
    m.fs.RO1a.width.setub(10)

    m.fs.RO1b_width_eq = Constraint(
        expr=m.fs.RO1b.width == m.fs.RO1a.width # stage widths are the same
    )
    ...
    m.fs.eq_minimum_water_flux1 = Constraint(
        expr=m.fs.RO1b.flux_mass_phase_comp[0, 1, "Liq", "H2O"]
...
def display_design(m):
    ...
    print("---RO1---")
    print("RO1 operating pressure %.1f bar" % (m.fs.RO1a.inlet.pressure[0].value / 1e5))
    print("RO1 membrane area %.1f m2" % (m.fs.RO1a.area.value))
    print("RO1 operating pressure %.1f bar" % (m.fs.RO1b.inlet.pressure[0].value / 1e5))
    print("RO1 membrane area %.1f m2" % (m.fs.RO1b.area.value))
...
def display_state(m):
    ...
    print_state("RO1 perm   ", m.fs.RO1b.permeate)
    print_state("RO1 reten  ", m.fs.RO1b.retentate)
